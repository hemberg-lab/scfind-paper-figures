...
df.1f$method <- factor(df.1f$method, levels = c('scfind', 'sce', 'seurat', 'loom'))

p.1f <- ggplot(df.1f, aes(x = as.character(no_of_markers), y = as.numeric(time), colour = method))  + 
    main + 
    labs(x = "Number of genes", y = "Query time (sec)") + 
    scale_colour_manual(values=tritPalette, labels = c("SCfind", "SCE", "Seurat", "Loom")) +
...
plot_grid(NULL, 
          ...
          NULL, 
          NULL,
          plot_grid(get_legend(p.1b + guides(col = guide_legend(ncol = 2)) + theme(legend.key.size = unit(.5, "cm"))),
                    get_legend(p.1c.left + guides(col = guide_legend(ncol = 2)) + theme(legend.title=element_blank(), legend.key.size = unit(.5, "cm"))),
                    nrow = 2, align = "v"),
          labels=c('B'), 
